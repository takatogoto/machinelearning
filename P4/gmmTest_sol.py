...
for i in init:
    ...
    np.savez('results_sol/gmm_toy_{}.npz'.format(i), iterations=iterations,
    ...
    for component in range(n_cluster):
        ...
        fig.ax.add_artist(e)
    fig.savefig('plots_sol/gmm_toy_dataset_{}.png'.format(i))
...
for i in init:
    ...
    im = (im > 0) * im
    plt.imsave('plots_sol/means_{}.png'.format(i), im, cmap='Greys')
    ...
    im = (im > 0) * im
    plt.imsave('plots_sol/samples_{}.png'.format(i), im, cmap='Greys')

    np.savez('results_sol/gmm_digits_{}.npz'.format(i), iterations=np.array(
