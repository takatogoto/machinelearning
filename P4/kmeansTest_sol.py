...
from kmeans_sol import KMeans, KMeansClassifier
...
def kmeans_toy():
    ...
    fig.savefig('plots_sol/toy_dataset_real_labels.png')

    fig.ax.scatter(x[:, 0], x[:, 1])
    fig.savefig('plots_sol/toy_dataset.png')
    n_cluster = 4
    ...
    fig.savefig('plots_sol/toy_dataset_predicted_labels.png')

    np.savez('results_sol/k_means_toy.npz',
...
def kmeans_image_compression():
    ...
    plt.imsave('plots_sol/compressed_baboon.png', new_im)

    np.savez('results_sol/k_means_compression.npz', im=im, centroids=centroids,
...
def kmeans_classification():
    ...
    plt.imsave('plots_sol/digits.png', im, cmap='Greys')
    ...
    np.savez('results_sol/k_means_classification.npz',
